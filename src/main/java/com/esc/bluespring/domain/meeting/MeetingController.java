...
public class MeetingController {
    ...
    @GetMapping
    @Operation(description = "메인 화면 과팅 목록, 내 지역만 보기는 Authorization 헤더 필요", parameters = @Parameter(in = ParameterIn.HEADER, name = "Authorization"))
    public BaseResponse<CustomSlice<MainPageListElement>> search(
        @ParameterObject @Valid SearchCondition condition, Member user,
        @ParameterObject Pageable pageable) {
        if (!(user instanceof Student) && condition.isMyLocation() != null
            && condition.isMyLocation()) {
    ...
    public BaseResponse<List<ListElement>> getList(SearchCondition condition) {
        List<Meeting> list = meetingService.getList(condition.ids());
        return new BaseResponse<>(list.stream().map(meetingMapper::toListElement).toList());
    }

    @GetMapping("{id}")
    @Operation(description = "홈 과팅 목록에서 과팅 상세보기", parameters = @Parameter(in = ParameterIn.HEADER, name = "Authorization"))
    public BaseResponse<Detail> getDetail(@PathVariable UUID id, Member user,
        ...
        return new BaseResponse<>(
            meetingMapper.toDetail(meeting, user instanceof Student student ? student : null));
    }

    @GetMapping("me")
    @RolesAllowed({STUDENT})
    @Operation(description = "내 과팅 목록", parameters = @Parameter(required = true, in = ParameterIn.HEADER, name = "Authorization"))
    public BaseResponse<CustomSlice<MyMeetingPageListElement>> search(Student student,
                                                                      @ParameterObject Pageable pageable) {
    ...
    public BaseResponse<Boolean> create(@Valid @RequestBody Create dto, Student member) {
    ...
    public BaseResponse<Boolean> request(@PathVariable UUID id,
                                         @Valid @RequestBody MeetingDto.Request dto,
                                         Student member) {
    ...
    public BaseResponse<CustomSlice<MeetingRequestDto.Detail>> searchRequestsWithMeeting(
        @PathVariable UUID id, MeetingRequestDto.SearchCondition condition, Member user,
        Pageable pageable) {
        Meeting meeting = meetingService.find(id);
    ...
    public BaseResponse<Boolean> addWatchlist(@PathVariable UUID id, Student member) {
    ...
    public BaseResponse<Boolean> takeOutFromWatchlist(@PathVariable UUID id, Student member) {
