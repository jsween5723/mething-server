...
public class MemberController {
    ...
    public JwtToken refresh(HttpServletRequest request, HttpServletResponse response) {
        RefreshToken refresh = refreshTokenService.refresh(
            getRefreshTokenFromCookie(request));
        Cookie refreshToken = new Cookie("refreshToken", refresh.getRefreshToken().toString());
        refreshToken.isHttpOnly();
        refreshToken.setSecure(true);
        response.addCookie(refreshToken);
        return new JwtToken(customJwtEncoder.generateAccessToken(refresh.getMember()));
    }
    ...
    private UUID getRefreshTokenFromCookie(HttpServletRequest request) {
        ...
        if (cookies.isEmpty() || cookies.get(0).getValue() == null || cookies.get(0).getValue().isBlank()) {
